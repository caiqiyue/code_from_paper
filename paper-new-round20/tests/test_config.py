...
class PaperNewSelectorConfigTests(unittest.TestCase):
    ...
    def test_config_path_resolution_works_from_project_root_and_paper_new_root(self):
        repo_dir_name = Path(__file__).resolve().parents[1].name
        resolved_prefixed = resolve_config_path(
            f"{repo_dir_name}/configs/single_node_jobs_selector.yaml"
        )
    ...
    def test_round19_guard_configs_are_reduced_scale_while_full_run_keeps_screening_scale(self):
        guard = load_yaml_config(
        ...
        self.assertEqual(
            guard["selector"]["seed_budget_rule"]["mode"],
            "hierarchical_shape_routing",
        )
        self.assertEqual(guard["data"]["train_limit"], 128)
        self.assertEqual(guard["data"]["eval_limit"], 128)
        self.assertEqual(guard["data"]["initialization_limit"], 512)
        self.assertEqual(guard["bootstrap"]["num_prompts"], 40)
        self.assertEqual(guard["eval"]["small_epochs"], 4)

        self.assertEqual(full["data"]["train_limit"], 256)
        self.assertEqual(full["data"]["eval_limit"], 256)
        self.assertEqual(full["data"]["initialization_limit"], 1024)
    ...
    def test_round20_uncertain_compare_uses_low_footprint_shared_gpu_profile(self):
        ...
        self.assertEqual(config["llm"]["generator"]["gpu_memory_utilization"], 0.28)
        self.assertEqual(config["llm"]["generator"]["startup_required_free_gb"], 2)
