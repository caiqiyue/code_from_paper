...
class _RetryableCacheFailureGenerator:
...
class Stage1RunnerReleaseTests(unittest.TestCase):
    def test_stage1_runner_retries_once_after_retryable_vllm_cache_block_failure(self):
        fake_backend = _FakeTextBackend()
        fake_embedder = _FakeEmbedder()
        retrying_generator = _RetryableCacheFailureGenerator()
        config = {
        ...
        self.assertEqual(retrying_generator.calls, 2)
        self.assertEqual(release_runtime.call_args_list[0].args, (fake_backend,))
        self.assertEqual(release_runtime.call_args_list[-1].args, (fake_backend, fake_embedder))
    ...
    def test_stage1_runner_uses_hierarchical_shape_routing_seed_budget_path(self):
        fake_backend = _FakeTextBackend()
        fake_embedder = _FakeEmbedder()
        config = {
            "pipeline": {"stage1_mode": "selector_seed_search"},
        ...
        with _round_context_patch(), patch(
            ...
            return_value={
                ...
                "seed_budget_summary": {
                    ...
                    "regime": "broad_tail",
                    "shape_score": 1.2,
                    "descriptor": {"median_len": 205.0, "p75_len": 396.0},
                    ...
                    "utility_gap": 0.01,
                    "tiebreak_applied": False,
                    "tiebreak_reason": "policy_resolution",
                    "rule": {"enabled": True, "mode": "hierarchical_shape_routing"},
                },
            },
        ) as calibration_mock, patch(
            "paper_new_selector.stage1_runner.greedy_select_candidates",
        ...
        calibration_mock.assert_called_once()
        greedy_mock.assert_not_called()
        self.assertEqual(summary["seed_budget"]["mode"], "hierarchical_shape_routing")
        self.assertEqual(summary["seed_budget"]["regime"], "broad_tail")
        self.assertEqual(summary["seed_budget"]["resolved_seed_top_k"], 22)
        self.assertEqual(summary["seed_budget"]["selection_stage"], "broad_tail_policy")

    def test_stage1_runner_exposes_arbitration_fields_from_hierarchical_summary(self):
        fake_backend = _FakeTextBackend()
        fake_embedder = _FakeEmbedder()
        config = {
            "pipeline": {"stage1_mode": "selector_seed_search"},
        ...
        sample_bundle = {
            "train_samples": [
                _FakeSample("alpha beta gamma"),
                _FakeSample("delta epsilon zeta"),
            ],
            "eval_samples": [_FakeSample("eta theta iota")],
            "init_samples": [_FakeSample("seed sample text")],
        }
        decision = SimpleNamespace(
            selected_indices=[0],
            hard_negative_indices=[1],
            hard_negative_reason={1: "boundary_negative"},
            accept_scores=[0.8, 0.2],
            to_dict=lambda: {"selected_indices": [0], "hard_negative_indices": [1]},
        )
        ...
        with _round_context_patch(), patch(
            ...
            return_value={
                ...
                "seed_budget_summary": {
                    ...
                    "arbitration_winner_policy": "compact_structured",
                    "arbitration_reason": "support",
                    "arbitration_broad_budget": 22,
                    "arbitration_compact_budget": 19,
                },
            },
        ) as calibration_mock, patch(
            "paper_new_selector.stage1_runner.greedy_select_candidates",
        ...
        greedy_mock.assert_not_called()
        self.assertEqual(
            summary["seed_budget"]["selection_stage"],
            "uncertainty_policy_arbitration",
        )
        self.assertTrue(summary["seed_budget"]["arbitration_triggered"])
        self.assertEqual(
            summary["seed_budget"]["arbitration_winner_policy"],
            "compact_structured",
        )
