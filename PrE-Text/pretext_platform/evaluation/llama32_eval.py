...
No Accelerator to avoid segfault issues on Windows.
"""
...
def run_llama32_eval(
    ...
    No Accelerator to avoid segfault issues on Windows with large models.
    """
    ...
    # Use LlamaTokenizer for Llama models (same as bootstrap uses)
    tokenizer = LlamaTokenizer.from_pretrained(llama32_path, local_files_only=True)
    # Handle padding token
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token

    # Load model to CPU first, then move to GPU to avoid segfault on Windows
