"""Downstream next-token evaluation with DistilGPT2 on synthetic data."""
...
def evaluate(model, eval_loader, accelerator, xent_loss):
    ...
    with torch.no_grad():
        for batch in eval_loader:
            ...
            valid_mask = flat_labels != -100  # Ignore padded positions in both loss and accuracy.
            ...
            for k in top_k_accuracies:
                top_k_accuracies[k] += correct_predictions[:, :k].sum()  # Count whether the gold token is inside the top-k set.
...
def main(args, output_dir, train_texts, eval_texts):
    ...
    total_epochs = 20
    batch_size = 256
    log_dir = os.path.join(output_dir, "log_models_and_accuracies")
    ...
    def tokenize(example):
        ...
        sent = [tokenizer.bos_token] + sent + [tokenizer.eos_token]  # Add explicit sequence boundaries.
    ...
    model.load_state_dict(checkpoint["model_state_dict"])  # Warm-start from the C4-pretrained DistilGPT2 checkpoint.
