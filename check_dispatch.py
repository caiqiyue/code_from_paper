#!/usr/bin/env python3
"""Check what from_pretrained does with quantization_config."""
import subprocess
import os
import tempfile
...
$cmd = "source /root/miniconda3/etc/profile.d/conda.sh && cd /root/autodl-tmp/caiqiyue/code_from_paper/PrE-Text && conda activate pretext && python -c 'from transformers import AutoModelForCausalLM; import inspect; src = inspect.getsource(AutoModelForCausalLM.from_pretrained); lines = src.split(chr(10)); dispatch_line = [l for l in lines if \"dispatch_model\" in l]; print(dispatch_line)'" 2>&1
...
ps1_path = os.path.join(tempfile.gettempdir(), 'check_dispatch.ps1')
