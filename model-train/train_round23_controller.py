...
from common import DEFAULT_ROUND23_DATASET_DIR, DEFAULT_ROUND23_MODEL_DIR, DEFAULT_ROUND23_SPLIT_DIR, dump_json, load_yaml, read_jsonl
from features import extract_feature_matrix_with_fields
from round23_controller_models import build_regressor, model_file_extension, predict_regressor, require_model_family, save_regressor
...
def train_controller_models(
    *,
    ...
    delta_actions = [int(value) for value in model_cfg.get("delta_actions", [-2, -1, 0, 1, 2])]
    ...
    selected_params_by_action: dict[str, Any] = {}
    for delta_k in delta_actions:
        action_rows = [row for row in dev_rows if int(row["action_delta_k"]) == int(delta_k)]
        ...
        for candidate in candidate_param_sets:
            ...
            for fold in folds_payload["folds"]:
                ...
                preds = predict_regressor(family=normalized_family, model=reg, feature_matrix=val_x) if val_x else []
                mse = (
        ...
        if best_candidate is None:
            raise RuntimeError(f"No valid candidate parameters produced for delta_k={delta_k}")
        cv_results[str(delta_k)] = candidate_results
        selected_params_by_action[str(delta_k)] = best_candidate
    ...
    for delta_k in delta_actions:
        ...
        best_params = dict(selected_params_by_action[str(delta_k)]["params"])
        ...
        model_path = output_root / f"model_dk_{action_name}{extension}"
        save_regressor(family=normalized_family, model=reg, path=model_path)
        trained_models[str(delta_k)] = str(model_path)
        ...
        feature_importance_payload[str(delta_k)] = importance_payload
    ...
    report = {
        ...
        "selected_params_by_action": selected_params_by_action,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train round23 controller models.")
...
def main() -> int:
    ...
    print(
        f"TRAINED round23 family={report['model_family']} feature_version={report['feature_version']} "
        f"actions={len(report['delta_actions'])} train_rows={report['train_row_count']}"
    )
    return 0
