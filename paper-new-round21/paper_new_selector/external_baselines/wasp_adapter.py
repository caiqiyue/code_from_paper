...
def clamp_text_budget(texts: list[str], *, budget: int) -> list[str]:
    cleaned: list[str] = []
    seen: set[str] = set()
    for raw_text in texts:
        ...
        seen.add(text)
        cleaned.append(text)
    return cleaned[: max(0, int(budget))]


def build_wasp_stage1_summary(*, texts: list[str], budget: int) -> dict:
    final_texts = clamp_text_budget(texts, budget=budget)
    return {
        "mode": "wasp_adapter",
        "selected_indices": [],
