...
def _build_budget_decisions_and_metrics(
    *,
    ...
    return candidate_seed_top_k, decisions_by_budget, enriched_metrics


def _build_generic_selected_budget_summary(
    *,
    selector_cfg: dict[str, Any],
    calibration_cfg: dict[str, Any],
    mode: str,
    candidate_seed_top_k: list[int],
    enriched_metrics: dict[int, dict[str, Any]],
    ...
    ranked_by_utility = sorted(
        candidate_seed_top_k,
        key=lambda budget: (
            float(enriched_metrics[int(budget)].get("utility", 0.0)),
            -int(budget),
        ),
        reverse=True,
    )
    default_runner_up = (
        int(ranked_by_utility[1])
        if len(ranked_by_utility) > 1
        else int(resolved_seed_top_k)
    )
    selected_utility = float(
        selected.get(
            "selected_utility",
            enriched_metrics[resolved_seed_top_k].get("utility", 0.0),
        )
    )
    runner_up_seed_top_k = int(
        selected.get("runner_up_seed_top_k", default_runner_up)
    )
    runner_up_utility = float(
        selected.get(
            "runner_up_utility",
            enriched_metrics[int(runner_up_seed_top_k)].get("utility", selected_utility),
        )
    )
    utility_gap = float(selected.get("utility_gap", selected_utility - runner_up_utility))
    seed_budget_summary = {
        "configured_seed_top_k": int(selector_cfg["seed_top_k"]),
        "resolved_seed_top_k": resolved_seed_top_k,
        "rule": calibration_cfg,
        "mode": mode,
        "candidate_seed_top_k": list(candidate_seed_top_k),
        "per_budget_metrics": {
            str(budget): enriched_metrics[int(budget)]
            for budget in candidate_seed_top_k
        },
        "selected_utility": selected_utility,
        "runner_up_seed_top_k": runner_up_seed_top_k,
        "runner_up_utility": runner_up_utility,
        "utility_gap": utility_gap,
        "tiebreak_applied": bool(selected.get("tiebreak_applied", False)),
        "tiebreak_reason": str(selected.get("tiebreak_reason", "policy_resolution")),
    }
    for key in (
        ...
        "policy_feasible_budgets",
        "arbitration_triggered",
        "arbitration_winner_policy",
        "arbitration_reason",
        "arbitration_broad_budget",
        "arbitration_compact_budget",
        "arbitration_broad_stability",
        "arbitration_compact_stability",
    ):
        if key in selected:
            seed_budget_summary[key] = selected[key]
    return seed_budget_summary


def resolve_seed_top_k_by_hierarchical_routing(
    *,
    selector_cfg: dict[str, Any],
    candidate_vectors: list[list[float]],
    candidate_texts: list[str],
    ...
    calibration_cfg = dict(selector_cfg.get("seed_budget_rule", {}))
    candidate_seed_top_k, decisions_by_budget, enriched_metrics = (
        _build_budget_decisions_and_metrics(
    ...
    if selected.get("resolved_seed_top_k") is None:
        policies_cfg = dict(calibration_cfg.get("policies", {}))
        uncertain_cfg = dict(policies_cfg.get("uncertain", {}))
        compact_policy = dict(policies_cfg.get("compact_structured", {}))
        fallback_mode = str(
            uncertain_cfg.get("fallback_mode", "self_calibrated_constrained")
        )
        fallback_cfg = {
            "coverage_constraint": dict(
                uncertain_cfg.get(
                    "coverage_constraint",
                    calibration_cfg.get("coverage_constraint", {}),
                )
            ),
            "utility": dict(
                compact_policy.get("utility", calibration_cfg.get("utility", {}))
            ),
        ...
        if fallback_mode == "self_calibrated":
            fallback = select_budget_with_recheck(
                metrics_by_budget=enriched_metrics,
                calibration_cfg=fallback_cfg,
            )
        elif fallback_mode == "self_calibrated_constrained":
            fallback = select_budget_by_constrained_utility(
                metrics_by_budget=enriched_metrics,
                calibration_cfg=fallback_cfg,
            )
        else:
        ...
        fallback["regime"] = str(selected.get("regime", "uncertain"))
        fallback["shape_score"] = float(selected.get("shape_score", 0.0))
        fallback["descriptor"] = dict(selected.get("descriptor", {}))
        fallback["selection_stage"] = (
            f"{selected.get('selection_stage', 'policy_resolution')}_global_fallback"
        )
        fallback["policy_fallback_used"] = True
        fallback["policy_fallback_reason"] = "no_policy_feasible_budget"
        fallback["policy_feasible_budgets"] = list(selected.get("feasible_budgets", []))
        selected = fallback

    resolved_seed_top_k = int(selected["resolved_seed_top_k"])
    decision = decisions_by_budget[resolved_seed_top_k]
    seed_budget_summary = _build_generic_selected_budget_summary(
        selector_cfg=selector_cfg,
        calibration_cfg=calibration_cfg,
