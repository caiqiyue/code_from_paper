...
class BudgetCalibrationTests(unittest.TestCase):
    ...
    def test_hierarchical_shape_routing_summary_includes_regime_and_descriptor(self):
        from types import SimpleNamespace
        from unittest.mock import patch

        decision_by_budget = {
            18: SimpleNamespace(selected_indices=[0], to_dict=lambda: {"selected_indices": [0]}),
            21: SimpleNamespace(selected_indices=[0, 1], to_dict=lambda: {"selected_indices": [0, 1]}),
            22: SimpleNamespace(selected_indices=[1, 2], to_dict=lambda: {"selected_indices": [1, 2]}),
        }

        def _fake_greedy_select_candidates(**kwargs):
            return decision_by_budget[int(kwargs["seed_top_k"])]

        with patch(
            "paper_new_selector.budget_calibration.greedy_select_candidates",
            side_effect=_fake_greedy_select_candidates,
        ):
            result = resolve_seed_top_k_by_hierarchical_routing(
                selector_cfg={
                    "seed_top_k": 20,
                    "lambda_generic": 0.0,
                    "lambda_redundancy": 0.0,
                ...
                candidate_vectors=[[1.0, 0.0], [0.9, 0.1], [0.0, 1.0]],
                candidate_texts=["alpha", "beta", "gamma"],
                private_vectors=[[1.0, 0.0], [0.8, 0.2], [0.0, 1.0]],
                private_lengths=[150, 205, 396],
                private_support=[1.0, 0.92, 0.40],
                genericity_penalty=[0.0, 0.1, 0.2],
            )

        summary = result["seed_budget_summary"]
        self.assertEqual(summary["mode"], "hierarchical_shape_routing")
        self.assertEqual(summary["regime"], "broad_tail")
        self.assertIn(summary["resolved_seed_top_k"], [21, 22])
        self.assertIn("descriptor", summary)
        self.assertIn("shape_score", summary)
        self.assertEqual(summary["selection_stage"], "broad_tail_policy")
