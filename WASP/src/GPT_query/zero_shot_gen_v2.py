...
if __name__ == "__main__":
    ...
    for im in range(len(args.llms)):        
        args.gen_output_dir = f'../data_new/{args.task_name}/{args.llm[im]}/{args.gen_total_num[im]}/' # "The output directory to which the generated dataset is saved"
        if not os.path.exists(args.gen_output_dir):
            os.makedirs(args.gen_output_dir)
        args.gen_task_file = f'../tasks/{args.task_name}/{args.task_name}-x2.json' # "A json file providing the instructions and other information required for dataset generation. "
        args.gen_model_name = args.llms[im] # "The pretrained model to use for dataset generation. Currently, only variants of GPT2 are supported."
        for _ in range(200):
