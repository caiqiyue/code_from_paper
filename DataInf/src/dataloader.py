"""Utilities for building noisy GLUE dataloaders used in classification experiments."""
...
def flip_label(example, ind, noise_index):
    """Flip the label of a training sample when its index is marked as noisy."""
    if ind in noise_index:
        example["label"] = 1 - example["label"]
    return example

def load_noisy_dataset_by_task(task="mrpc", noise_ratio=0.1):
    """Load a GLUE split, optionally downsample it, and inject label noise."""
    glue_datasets = load_dataset("glue", task)  # Download/load the requested GLUE subset.
    ...
    if n_train > 4500 and n_val > 500:
        ...
        glue_datasets['train'] = glue_datasets['train'].select(new_n_train_list)  # Keep training cost manageable.
        glue_datasets['validation'] = glue_datasets['validation'].select(new_n_val_list)  # Match paper-sized eval set.
    ...
    if noise_ratio > 0.0:
        noise_index = np.random.choice(n_train,
                                       ...
                                       replace=False)  # Pick samples whose labels will be flipped.
    else:
        noise_index = []
...
def create_dataloaders(model_name_or_path="roberta-large",
                       ...
                       noise_ratio=0.1,
                       batch_size=32):
    """Tokenize a GLUE task and build train/eval dataloaders plus noise metadata."""
    ...
    def tokenize_function(examples, max_length=128):
        """Tokenize single-sentence or sentence-pair GLUE inputs."""
    ...
    def collate_fn(examples):
        """Pad a variable-length batch into a PyTorch tensor batch."""
        return tokenizer.pad(examples, padding="longest", return_tensors="pt")  
        
    train_dataloader = DataLoader(tokenized_datasets["train"],
                                  shuffle=True, 
                                  collate_fn=collate_fn,
                                  batch_size=batch_size)  # Shuffle to mimic standard fine-tuning.
    eval_dataloader = DataLoader(tokenized_datasets["validation"], 
                                 shuffle=False, 
                                 collate_fn=collate_fn, 
                                 batch_size=batch_size)  # Keep validation order stable for analysis.
