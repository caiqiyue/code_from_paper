...
def calculate_metric(predictions, metric_name):
    if metric_name == "accuracy":
        ...
        # Group predictions by correct_candidate
        candidate_groups = defaultdict(list)

        for pred in predictions:
            if isinstance(pred.correct_candidate, list):
                for candidate in pred.correct_candidate:
                    candidate_groups[candidate].append(
                        pred.predicted_candidate in pred.correct_candidate
                    )
            else:
                candidate_groups[pred.correct_candidate].append(
                    pred.correct_candidate == pred.predicted_candidate
                )

        # Calculate accuracy per correct_candidate
        accuracy_per_candidate = {
            candidate: np.mean(accuracies)
            for candidate, accuracies in candidate_groups.items()
        }
        return accuracy_per_candidate
    elif metric_name == "em":
