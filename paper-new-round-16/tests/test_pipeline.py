...
class PipelineWriteTests(unittest.TestCase):
    ...
    def test_run_pipeline_writes_budget_calibration_file_for_constrained_mode(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_root = Path(tmp)
            config = {
                "pipeline": {"stage1_mode": "selector_seed_search", "stage2_mode": "bootstrap"},
    ...
    def test_run_pipeline_writes_budget_calibration_file_for_hybrid_mode(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_root = Path(tmp)
            config = {
                "pipeline": {"stage1_mode": "selector_seed_search", "stage2_mode": "bootstrap"},
