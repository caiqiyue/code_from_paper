...
def _stringify_feasible_budgets(value: object) -> str:
    if value is None:
        return "NA"
...
def main() -> int:
    if len(sys.argv) != 5:
        raise SystemExit(
            "usage: append_round17_summary.py SUMMARY_PATH EXPERIMENT STATUS OUTPUT_DIR"
        )
    ...
    row = [
        experiment,
        status,
        calibration.get("resolved_seed_top_k", calibration.get("selected_seed_top_k", "NA")),
        calibration.get("runner_up_seed_top_k", "NA"),
        calibration.get("utility_gap", "NA"),
        _stringify_feasible_budgets(coverage_constraint.get("feasible_budgets")),
        coverage_constraint.get("threshold", "NA"),
        coverage_constraint.get("best_coverage_p25", "NA"),
