...
def main() -> int:
    if len(sys.argv) != 5:
        raise SystemExit(
            "usage: append_round18_summary.py SUMMARY_PATH EXPERIMENT STATUS OUTPUT_DIR"
        )
    ...
    row = [
        ...
        coverage_constraint.get("mode", "NA"),
        _stringify(coverage_constraint.get("feasible_budgets")),
        _stringify(coverage_constraint.get("metrics")),
        _stringify(coverage_constraint.get("family_score_by_budget")),
