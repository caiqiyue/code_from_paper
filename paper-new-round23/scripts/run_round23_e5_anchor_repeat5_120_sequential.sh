...
LOCK_DIR="$LOCK_ROOT/e5_anchor_repeat5_120.lock"
mkdir -p "$LOCK_ROOT"
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  LOCK_PID="$(cat "$LOCK_DIR/pid" 2>/dev/null || true)"
  if [[ -n "$LOCK_PID" ]] && kill -0 "$LOCK_PID" 2>/dev/null; then
    echo "Another E5 anchor repeat5 run is already active: pid=$LOCK_PID"
    exit 3
  fi
...
python paper-new-round23/scripts/generate_round23_e5_experiment_configs.py \
  --mode e5_anchor_k19_keepk0_all6_repeat5 \
  --mode e5_anchor_k19_round23_all6_repeat5 \
  --mode e5_anchor_k21_keepk0_all6_repeat5 \
  --mode e5_anchor_k21_round23_all6_repeat5
