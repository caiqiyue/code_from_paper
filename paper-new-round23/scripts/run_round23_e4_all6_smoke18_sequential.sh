...
LOCK_DIR="$LOCK_ROOT/e4_all6_smoke18.lock"
mkdir -p "$LOCK_ROOT"
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  LOCK_PID="$(cat "$LOCK_DIR/pid" 2>/dev/null || true)"
  if [[ -n "$LOCK_PID" ]] && kill -0 "$LOCK_PID" 2>/dev/null; then
    echo "Another E4 all6 smoke run is already active: pid=$LOCK_PID"
    exit 3
  fi
...
python paper-new-round23/scripts/generate_round23_e4_experiment_configs.py \
  --mode e4_a_oneshot_all6_smoke \
  --mode e4_b_keepk0_all6_smoke \
  --mode e4_c_three_round_stress_all6_smoke
