#!/usr/bin/env python3
"""Dynamic delta-k controller inference for round23 runtime."""
...
class ControllerBundle:
    def __init__(self, bundle_dir: str | Path):
        ...
        self.learner_family = str(
            self.metadata.get("learner_family", self.metadata.get("model_family", "lightgbm"))
        ).lower()
    ...
    def _load_metadata(self) -> dict[str, Any]:
        ...
        action_space = metadata.get("action_space", metadata.get("delta_actions"))
        if action_space is not None and list(action_space) != ACTIONS:
            raise ValueError(f"Unexpected action_space in metadata: {action_space}")
        return metadata
    ...
    def _load_models(self) -> dict[int, Any]:
        if self.learner_family == "lightgbm":
            ...
            return {
                delta_k: lgb.Booster(model_file=str(self._find_model_file(stem)))
        ...
        if self.learner_family in {
            ...
            for delta_k, stem in ACTION_TO_STEM.items():
                with self._find_model_file(stem).open("rb") as handle:
                    models[delta_k] = pickle.load(handle)
            return models
    ...
    def predict(self, feature_vector: list[float]) -> dict[int, float]:
        ...
        for delta_k, model in self.models.items():
            if self.learner_family == "lightgbm":
    ...
    def predict_action(self, feature_vector: list[float], reference_budget: int = 20) -> dict[str, Any]:
        ...
        best_delta_k = max(rewards, key=lambda delta_k: rewards[delta_k])
        target_budget = max(1, reference_budget + int(best_delta_k))
        sorted_rewards = sorted(rewards.items(), key=lambda item: item[1], reverse=True)
        confidence_margin = (
            float(sorted_rewards[0][1] - sorted_rewards[1][1]) if len(sorted_rewards) > 1 else 0.0
        )
        return {
            "predicted_delta_k": int(best_delta_k),
            "predicted_target_budget": int(target_budget),
