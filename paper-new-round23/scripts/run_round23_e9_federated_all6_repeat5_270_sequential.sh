...
export ROUND23_E9_PYTHON_BIN="$PYTHON_BIN"
...
LOCK_DIR="$LOCK_ROOT/e9_federated_all6_once_54.lock"
mkdir -p "$LOCK_ROOT"
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  LOCK_PID="$(cat "$LOCK_DIR/pid" 2>/dev/null || true)"
  if [[ -n "$LOCK_PID" ]] && kill -0 "$LOCK_PID" 2>/dev/null; then
    echo "Another E9 federated all6 once run is already active: pid=$LOCK_PID"
    exit 3
  fi
...
cd "$ROOT"
"$PYTHON_BIN" paper-new-round23/scripts/generate_round23_e9_federated_experiment_configs.py \
  --mode e9_f4_uniform_all6_once \
  --mode e9_f4_noniid_all6_once \
  --mode e9_f8_imbalance_noniid_all6_once
...
do
  ...
  "$PYTHON_BIN" paper-new-round23/scripts/round23_dynamic_experiment_runner.py \
    --mode "$MODE" \
    "${MODE_ARGS[@]}" \
    "$@"
  RUN_STATUS=$?
