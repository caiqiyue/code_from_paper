...
if (Test-Path $localFile) {
    ...
    Get-Content $localFile | Select-Object -First 3
} else {
...
Write-Host ""
Write-Host "=== Step 3: Transfer via SSH ==="
$remoteFile = "/root/autodl-tmp/caiqiyue/code_from_paper/PrE-Text/pretext_platform/evaluation/llama2_eval.py"
$cmd = "cat /tmp/llama2_eval_b64.txt | base64 -d > $remoteFile"

# First scp the base64 file
Write-Host "Transferring base64 file..."
...
Write-Host ""
Write-Host "=== Step 4: Verify upload ==="
$verifyCmd = "ssh -o PreferredAuthentications=password -o PubkeyAuthentication=no -o NumberOfPasswordPrompts=1 -o ConnectTimeout=30 -o StrictHostKeyChecking=no -p 26732 root@connect.nmb1.seetacloud.com `"wc -l $remoteFile && grep -n 'device_map.*accelerator.device' $remoteFile | head -3`" 2>&1"
$verifyResult = Invoke-Expression $verifyCmd
Write-Host $verifyResult

Write-Host ""
Write-Host "=== Step 5: Modify config to load_in_8bit=False ==="
...
Write-Host ""
Write-Host "=== Step 6: Run PT-2 with load_in_8bit=False ==="
...
Write-Host ""
Write-Host "=== Step 7: Restore config ==="
...
ps1_path = os.path.join(tempfile.gettempdir(), 'run_pt2_fix3.ps1')
