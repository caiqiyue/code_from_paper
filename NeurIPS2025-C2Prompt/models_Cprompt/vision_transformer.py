...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, label=None, client_ids=None, task_ids=None, register_blk=-1, prompt=None, prompt_class=None, class_train_wrong=None, label_mapping=None, select_prompt_class=False, q=None, train=False, task_id=None, aq_k=None, ep_g=None, client_index=None, args=None):
        B = x.shape[0]
        x = self.patch_embed(x)
                        ...
                        if select_prompt_class:      
                            if prompt_class is not None:
                                ...
                                if random.random() < ran:
                                    # print("prompt1", prompt_class.shape)
                                    B_t = x.size(0)
                                    # print("label_mapping", label_mapping)
                                    # print("prompt_class", prompt_class.shape)
                                    prompt_class_s = prompt_class[client_ids, task_ids, label_mapping[int(label[0])]]#(num_prompts, embed_dim)


                                    if not torch.all(prompt_class_s == 0):
                                        if prompt_class_s.dim() == 2:  
                                            prompt_class_s = prompt_class_s.unsqueeze(0).expand(B_t, -1, -1)  # (B, num_prompts, embed_dim)
                                        
                                        if label_mapping[int(label[0])] not in class_train_wrong:
                                            # print("label_mapping[int(label[0])]", label_mapping[int(label[0])])
                                       
                                        
                                            x = torch.cat((x, prompt_class_s), dim=1)  # (B, num_patches+1+num_prompts, embed_dim)
                                        # x = torch.cat((prompt_class_s, x), dim=1) 
