...
class DualPrompt_hard:
    ...
    def train(self, ep_g, model_old, model_g, prompt_pool, label_mapping, class_train_wrong, index, task_id, args, group=1):
        ...
        if "full" in self.model.args.method and self.model.client_index != 0 and self.task_id_old != 0:
            ...
            for epoch in range(self.epochs):
                ...
                for step, (indexs, images, target) in enumerate(self.train_loader):
                    ...
                    if "classincremental" not in self.model.args.method:
                        loss_value, normal_loss, t_c2loss, prompt_loss, prelogits_disloss, output_disloss, ntd_loss, align_loss, mean_aqk_list = self._compute_loss(indexs, images, target, prompt_pool, label_mapping, ep_g, class_train_wrong, index, task_id, args)
                    else:
                        loss_value, normal_loss, t_c2loss, prompt_loss, prelogits_disloss, output_disloss, ntd_loss, align_loss, mean_aqk_list, logits_loss_for_divide, repre_loss_for_divide = self._compute_loss(indexs, images, target, args)
    ...
    def _compute_loss(self, indexs, imgs, targets, prompt_pool, label_mapping, ep_g, class_train_wrong, index, task_id, args):
        if isinstance(self.device, int):
        ...
        if self.prompt_flag == 'codap_2d_v2' and "classincremental" in self.model.args.method:
            logits, prompt_loss, prelogits_current, prompt_client_current, control_loss, mean_aqk_list, q_map, out_map, logits_loss_for_divide, repre_loss_for_divide = self.model(imgs, label=targets, client_ids=index, task_ids=task_id, train=True, prompt_pool=prompt_pool, class_train_wrong=class_train_wrong, select_prompt_class=True, label_mapping=label_mapping, device=self.device, args=args)
            
        elif self.prompt_flag == 'codap_2d_v2':
            logits, prompt_loss, prelogits_current, prompt_client_current, control_loss, mean_aqk_list, q_map, out_map = self.model(imgs, label=targets, client_ids=index, task_ids=task_id, train=True, prompt_pool=prompt_pool, class_train_wrong=class_train_wrong, select_prompt_class=True, label_mapping=label_mapping, device=self.device, args=args)
        else:
            logits, prompt_loss, prelogits_current, prompt_client_current, control_loss, mean_aqk_list, q_map, out_map = self.model(imgs, train=True, device=self.device, args=args)
        
        if self.old_model == None:
            if self.prompt_flag == 'cprompt':
                with torch.no_grad():
                    logits_global, _, prelogits_global, prompt_client_global, _, _, _, _ = self.old_round_model(imgs, label=targets, client_ids=index, task_ids=task_id, train=True, prompt_pool=prompt_pool, class_train_wrong=class_train_wrong, select_prompt_class=True, label_mapping=label_mapping, args=args)               
                t_c2loss = 0
                prelogits_disloss = positive_loss()(prelogits_current, prelogits_global)
        else:
            if self.prompt_flag == 'cprompt':
                with torch.no_grad():
                    logits_previous_task, _, prelogits_previous_task, prompt_client_previous, _, _, _, _ = self.old_model(imgs, label=targets, client_ids=index, task_ids=task_id, train=True, prompt_pool=prompt_pool, class_train_wrong=class_train_wrong, label_mapping=label_mapping, args=args)
                    _, _, prelogits_global, prompt_client_global, _, _, _, _ = self.old_round_model(imgs, label=targets, client_ids=index, task_ids=task_id, train=True, prompt_pool=prompt_pool, class_train_wrong=class_train_wrong, select_prompt_class=True, label_mapping=label_mapping, args=args)
        ...
        if "classincremental" in self.model.args.method:
            if self.model.ep_g > 0 and 'v2' in self.model.args.method and '_ntd' not in self.model.args.method:
                with torch.no_grad():
                    dg_logits, _, _, _, _, _, _, _, _, _ = self.old_round_model(imgs, train=True, device=self.device, args=args)
            ...
            if self.model.ep_g > 0 and 'v2' in self.model.args.method and '_ntd' not in self.model.args.method:
                with torch.no_grad():
                    dg_logits, _, _, _, _, _, _, _ = self.old_round_model(imgs, label=targets, client_ids=index, task_ids=task_id, train=True, prompt_pool=prompt_pool, class_train_wrong=class_train_wrong, select_prompt_class=True, label_mapping=label_mapping, device=self.device, args=args)
                ntd_loss = self._ntd_loss(logits, dg_logits, targets)
            elif self.model.ep_g > 0 and 'v2' in self.model.args.method and 'nor' in self.model.args.method:
                with torch.no_grad():
                    dg_logits, _, _, _, _, _, _, _ = self.old_round_model(imgs, label=targets, client_ids=index, task_ids=task_id, train=True, prompt_pool=prompt_pool, class_train_wrong=class_train_wrong, select_prompt_class=True, label_mapping=label_mapping, device=self.device, args=args)
        ...
        if self.prompt_flag == 'cprompt':
            ...
            if args.dataset == 'ImageNet_R':
                # w = 0
                w = 0.9 if ep_g < 6 else ((ep_g // 3) - 1) * 0.4 + 0.9 if ep_g < 8 else ((ep_g // 3) - 1) * 0.45 + 0.9 if ep_g < 11 else ((ep_g // 3) - 1) * 0.5 + 0.9
            else:

                w = 0.9 if ep_g < 6 else ((ep_g // 3) - 1) * 0.35 + 0.9 if ep_g < 8 else ((ep_g // 3) - 1) * 0.4 + 0.9 if ep_g < 11 else ((ep_g // 3) - 1) * 0.45 + 0.9

            total_loss = normal_loss + prompt_loss.sum() + control_loss + w * ntd_loss + 0.05 * align_loss
